...
const Home = () => {
  ...
  const handleCreateMeeting = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        code,
        teacher_name: teacherName || profile?.display_name || "Teacher",
        is_active: true,
        user_id: user.id,
      } as any)
      .select()
    ...
    else if (data) { navigate(`/meeting/${data.id}`); }
  };
  ...
  const handleJoinMeeting = async (e: React.FormEvent) => {
    ...
    else if (participant) { navigate(`/meeting/${meeting.id}?pid=${participant.id}`); }
  };
