...
const Login = () => {
  ...
  useEffect(() => {
    if (!pendingRedirect || !user || profileLoading) return;
    ...
    setPendingRedirect(false);
  }, [pendingRedirect, user, profileLoading, profile, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authService.loginWithJWT({ email, password });
      ...
      if (!result.success) {
        ...
        // Use the existing signIn method to trigger auth state change
