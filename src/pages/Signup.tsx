...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authService.signupWithJWT({
        email,
        password,
        displayName,
        role,
      ...
      if (!result.success) {
        ...
        // Use the existing signUp method to trigger auth state change
        const { error, profileError } = await signUp(email, password, displayName, role);
        ...
        if (error) {
          ...
          toast({
            ...
            description: profileError.message || "Please contact support or try logging in again.",
            variant: "destructive",
          });
        } else {
          ...
          toast({ title: "Account created", description: "Welcome to Smart Classroom!" });
